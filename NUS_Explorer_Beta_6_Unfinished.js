...
const playerA = update_position(the_orange_man, [400, 1500]); // 400, 1500
const playerB = update_position(the_space_invader, [0, 600]); // 0, 600
const playerC = update_position(nus_lion, [600, 1500]); // 600, 1500
const explorer = update_position(merlion, [1000, 1500]); // 1000, 1500
...
// ----------players activation------------------------
    if (pointer_over_gameobject(playerC) && input_left_mouse_down() && game_on) {
    ...
    if (pointer_over_gameobject(playerA) && input_left_mouse_down() && game_on) {
